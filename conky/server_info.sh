...
# Servers
declare -A servers=([alexov]='alexov@alexov' [home]='home@home');
...
function fetchInfoServer {
	...
	if [[ -z "$fileName" || $(( `date +%s` - `stat -L --format %Y $fileName` )) -gt "$refreshRate" ]] ; then
		...
		echo "$monitorFile" | ssh -q -p 22 ${servers[$1]} '[[ ! -f ".monitor.sh" ]] && cat > .monitor.sh; bash .monitor.sh' > $fileName
	fi
}
...
for i in $@; do
	case $i in
		...
		;;
		"loggedInUsers") 
      echo $(sed -n '/^Logged In/,/^Memory/{ /^Logged In/d; /^Memory/d; p }' < $fileName) 
    ;;
		"disks") 		 
      echo $(sed -n '/^Filesystem/,/^Docker/{ /^Filesystem/d; /^Docker/d; p }' < $fileName) 
    ;;
    "docker") 
      echo "$(sed -n '/^NAME/,$p'< $fileName)" 
    ;;
